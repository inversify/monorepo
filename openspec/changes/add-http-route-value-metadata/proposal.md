...
- Each adapter-specific package (Express, Express v4, Fastify, Hono, uWebSockets) will export its own `create<Framework>RouteValueMetadataUtils` that wraps the core factory and wires up the adapter-specific request-to-route resolution.
...
- `http-route-value-metadata-adapters`: Adapter-specific implementations for Express, Express v4, Fastify, Hono, and uWebSockets that resolve the route from a request and look up stored metadata. Includes the public `create<Framework>RouteValueMetadataUtils` factories.
