...
- `http-route-value-metadata-adapters`: Adapter-specific `_getRouteValueMetadataHandler` middleware implementations and per-adapter `createRouteValueMetadataUtils` exports (re-exports for Express, Express v4, Fastify, and uWebSockets; custom context-based implementation for Hono).
...
- **Public API additions**: Each adapter package gains a `createRouteValueMetadataUtils` export. `@inversifyjs/http-core` retains its base implementation.
