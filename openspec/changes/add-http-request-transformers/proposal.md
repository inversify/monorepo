...
- Add minimal core plumbing so `_buildRouter` receives per-route transformers: protected hook + mandatory `RouteParams` field whose value may be `undefined` when unused.
...
- **Packages modified**: `@inversifyjs/http-uwebsockets` (primary), `@inversifyjs/http-core` (minimal protected hook / `RouteParams` field only).
