...
- `http-route-value-metadata-adapters`: Adapter-specific `_getRouteValueMetadataHandler` middleware implementations and per-adapter `createRouteValueMetadataUtils` exports (re-exports for Express, Express v4, Fastify, and uWebSockets; `HonoRequest`-typed wrapper for Hono).
