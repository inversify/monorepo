...
// Container with disposal plugin
const container = new Container();
container.load(new DisposePlugin());

container.bind<DatabaseConnection>('Database').to(DatabaseConnection);

// Resources are automatically disposed when container is disposed
container.dispose();
```

#### Async Disposal
```typescript
class AsyncResource implements IAsyncDisposable {
  async disposeAsync(): Promise<void> {
    await this.cleanup();
  }
}

// Plugin handles async disposal automatically
const container = new Container();
container.load(new DisposePlugin({
  asyncTimeout: 5000 // 5 second timeout for async disposal
}));
```

#### Custom Disposal Strategies
```typescript
class CustomDisposePlugin extends DisposePlugin {
  protected createDisposalStrategy(instance: any): DisposalStrategy {
    if (hasCustomCleanup(instance)) {
      return new CustomDisposalStrategy(instance);
    }
    return super.createDisposalStrategy(instance);
  }
}
```
