...
class CacheService {
  constructor(
    @inject('Redis') @named('primary') private redis: Redis,
    @multiInject('CacheProvider') private providers: CacheProvider[]
  ) {}
}
...
class RequestHandler {
  constructor(
    @inject('Database') @tagged('region', 'us-east') private db: Database,
    @optional() @inject('Metrics') private metrics?: MetricsCollector
  ) {}
}
```

#### Conditional and Factory Bindings
```typescript
@injectable()
