...
describe(ContainerModule, () => {
  it('Should be able to set the registry of a container module', () => {
    ...
    // eslint-disable-next-line @typescript-eslint/no-unused-expressions, vitest/valid-expect
    expectTypeOf(typeof warriors.id).toBeNumber;
  });
  ...
  it('Should be able to add multiple async deactivation hook through a container module (async)', async () => {
    const onActivationHandlerMock: Mock<() => Promise<void>> = vitest.fn<
      () => Promise<void>
    >(async () => undefined);
    ...
    const containerModule: ContainerModule = new ContainerModule(
      async (options: ContainerModuleLoadOptions) => {
        options.onDeactivation(serviceIdentifier, onActivationHandlerMock);
        options.onDeactivation(serviceIdentifier, onActivationHandlerMock);
      },
    );
  ...
  it('should be able to unbindAsync from a module', async () => {
    ...
    //bindings removed
    expect(container.getAll(sid)).toStrictEqual([]);
  });
});
