...
describe('inRequestScope', () => {
  it('Should support request scope in basic bindings', () => {
    ...
    // One requests should use two instances because scope is transient
    expect(samurai2.primaryWeapon.use()).not.toBe(
    ...
    expect(samurai.primaryWeapon.use()).not.toBe(samurai2.primaryWeapon.use());
    expect(samurai.secondaryWeapon.use()).not.toBe(
      samurai2.secondaryWeapon.use(),
    );
    ...
    expect(samurai3.primaryWeapon.use()).not.toBe(samurai4.primaryWeapon.use());
    expect(samurai3.secondaryWeapon.use()).not.toBe(
  ...
  it('Should support request scope when using contraints', () => {
    ...
    expect(samurai1.secondaryWeapon.use()).toBe(samurai1.tertiaryWeapon.use());
    ...
    // Shuriken should be one shared instance because scope is request scope
    expect(samurai2.secondaryWeapon.use()).toBe(samurai2.tertiaryWeapon.use());

    // Two request should use one instances of Katana
    // for each instance of Samurai because scope is request scope
    expect(samurai1.secondaryWeapon.use()).not.toBe(
      samurai2.secondaryWeapon.use(),
    );
    expect(samurai1.tertiaryWeapon.use()).not.toBe(
