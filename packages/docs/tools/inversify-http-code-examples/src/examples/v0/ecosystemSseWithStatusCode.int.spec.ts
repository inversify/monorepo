...
import { EventsController } from './ecosystemSseWithStatusCode.js';
...
describe.each<[(container: Container) => Promise<Server>]>([
  ...
  'Ecosystem SSE (With Status Code)',
  (buildServer: (container: Container) => Promise<Server>) => {
    ...
    describe('when connecting to the SSE endpoint with custom status code', () => {
      let response: Response;

      beforeAll(async () => {
        const fetchRequest: Request = new Request(
          `http://${server.host}:${server.port.toString()}/events/custom-status`,
          {
            method: 'GET',
            mode: 'no-cors',
            redirect: 'follow',
      ...
      it('should return a 201 Created response (custom status code)', () => {
      ...
      describe('when reading the response', () => {
        ...
        it('should return the expected SSE formatted message', () => {
          const expectedMessage: string = 'data: Event with custom status\n\n';

          expect(responseText).toBe(expectedMessage);
        });
      });
    });
  },
