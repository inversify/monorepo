...
import { OperationIdController } from './decoratorApiOasOperationId.js';

describe.each<[(container: Container) => Promise<Server>]>([
  [buildExpressServer],
])(
  'Decorator API (OasOperationId)',
  (buildServer: (container: Container) => Promise<Server>) => {
    let server: Server;

    beforeAll(async () => {
      const container: Container = new Container();
      container.bind(OperationIdController).toSelf().inSingletonScope();
    ...
    it('should add operationId to operation', async () => {
      ...
      expect(operation?.operationId).toBe('getWelcomeMessage');
    });
  },
);
