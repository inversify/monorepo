...
The CLI prompts for a package manager (`npm`, `pnpm`, or `yarn`), an HTTP adapter (`express`, `fastify`, `hono`, or `uwebsockets`), and an API modeling style (`code-first` or `schema-first`). You can also pass them as flags:

<Tabs groupId="package-manager">
  <TabItem value="npm" label="npm">

```bash
npm create @inversifyjs/http@latest my-app -- --pm npm --adapter express --apiStyle code-first
```
...
```bash
pnpm create @inversifyjs/http my-app --pm pnpm --adapter express --apiStyle code-first
```
...
```bash
yarn create @inversifyjs/http my-app --pm yarn --adapter express --apiStyle code-first
```
