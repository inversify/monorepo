...
`@CaptureRequestValues(...)` is a method decorator that registers a request transformer taking a snapshot of the request values you list, before any `await` happens, and replaces the request with a Proxy serving those snapshots. Reads of the captured kinds then keep working after asynchronous work.

The available request value kinds are `'body'`, `'headers'`, `'method'`, `'params'`, `'query'` and `'url'`:

- **`'headers'`**: all headers, so `getHeader()`, `forEach()` and cookies reads work later.
- **`'method'`**: the HTTP method.
- **`'params'`**: all named route params, which also makes reading every route parameter at once possible on captured requests.
...
- **`'url'`**: the URL.
- **`'body'`**: raw body chunks are buffered by `installHttpResponseBodyCapture` and replayed to later readers such as `@Body()`. The transformer does not parse the body or read it asynchronously; it only installs buffering so later body reads can consume the same chunks.
...
Route parameter names cannot be read when the decorator is applied, because `@Controller` is a class decorator and therefore runs after method decorators. Instead, the capture transformer derives the names from the matched controller path and method path the first time it needs them for that route, and caches them per route path for every later request, so no path parsing happens per request. If neither controller nor method path metadata can be resolved at that point (for example when the method has no HTTP method decorator), the transformer fails with an error describing the missing metadata. If a method is mapped to multiple paths with different route parameters and the matched route cannot be identified, numeric `getParameter()` indexes cannot be resolved and the transformer fails with an error describing that case.
