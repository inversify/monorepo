...
- **`'url'`**: the URL.
- **`'body'`**: the parsed body. It is the only kind read asynchronously, and it is read after every synchronous snapshot is taken. Later body reads (for example `@Body()`) reuse the captured body instead of reading the request a second time.
...
Route parameter names cannot be read when the decorator is applied, because `@Controller` is a class decorator and therefore runs after method decorators. Instead, the capture transformer derives the names from the controller path and the method path the first time it needs them, and caches them for every later request, so no path parsing happens per request. If neither controller nor method path metadata can be resolved at that point (for example when the method has no HTTP method decorator), the transformer fails with an error describing the missing metadata.
