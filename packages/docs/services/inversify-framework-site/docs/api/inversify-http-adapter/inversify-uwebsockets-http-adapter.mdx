...
- **`url`**: the URL.
- **`body`**: also captures headers (needed for `content-type`). Raw body chunks are buffered and replayed to later readers such as `@Body()`. The transformer does not parse the body or read it asynchronously; it only installs buffering so later body reads can consume the same chunks.
