...
`CustomParameterDecoratorHandlerOptions` provides the following methods:

| Method | Signature | Description |
| --- | --- | --- |
| `getBody` | `(request, response, parameterName?) => unknown` | Reads the request body, optionally by parameter name. |
| `getCookies` | `(request, response, parameterName?) => unknown` | Reads cookies, optionally by parameter name. |
| `getHeaders` | `(request, parameterName?) => unknown` | Reads headers, optionally by parameter name. |
| `getMethod` | `(request) => string` | Returns the HTTP method (e.g. `GET`, `POST`). |
| `getParams` | `(request, parameterName?) => unknown` | Reads route parameters, optionally by parameter name. |
| `getQuery` | `(request, parameterName?) => unknown` | Reads query parameters, optionally by parameter name. |
| `getUrl` | `(request) => string` | Returns the request URL path including query string (e.g. `/users?page=1`). |
| `setHeader` | `(request, response, key, value) => void` | Sets a response header. |
| `setStatus` | `(request, response, statusCode) => void` | Sets the response status code. |
...
`CustomNativeParameterDecoratorHandlerOptions` extends `CustomParameterDecoratorHandlerOptions` with additional methods:

| Method | Signature | Description |
| --- | --- | --- |
| `send` | `(request, response, value) => TResult \| Promise<TResult>` | Sends a response body directly. |
| `sendBodySeparator` | `(request, response) => void \| Promise<void>` | Flushes response headers before streaming body chunks. |
