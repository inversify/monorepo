...
每当单例作用域服务被解绑时，就会分发停用事件。停用处理程序接收解析值并且不返回任何内容。
...
添加到容器的处理程序最先被解析。添加到子容器的任何处理程序都在添加到其父容器的处理程序之前调用。接下来调用容器中的相关绑定，最后调用 `preDestroy` 方法。在上面的示例中，相关绑定是绑定到已解绑的 "Destroyable" 服务标识符的那些绑定。
