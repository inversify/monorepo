...
Suites provides type-safe, automated unit testing for InversifyJS applications. It eliminates manual mock creation by
auto-generating fully typed test doubles, helping developers to write tests in seconds instead of minutes.
...
```bash
npm install -D @suites/unit @suites/di.inversify @suites/doubles.(jest|vitest|sinon)
```
...
Token-injected dependencies are always auto-mocked (read more about DIP):
...
describe('PaymentService', () => {
  ...
  beforeAll(async () => {
    ...
    database = unitRef.get('DATABASE');          // By token
    apiKey = unitRef.get('API_KEY');             // Primitive token
  });
});
```

### InversifyJS mMetadata Support
...
describe('Ninja', () => {
  ...
  beforeAll(async () => {
    ...
    katana = unitRef.get('Weapon', { canThrow: false });
    shuriken = unitRef.get('Weapon', { canThrow: true });
  });
...
Suites uses a **virtual DI container** that analyzes your InversifyJS classes without initializing an actual container.
