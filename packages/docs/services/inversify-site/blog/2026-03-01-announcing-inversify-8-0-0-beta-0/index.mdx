...
- Node.js 22 (current LTS) and Node.js 20 ship `require(esm)` — the ability to `require()` an ES module from a CommonJS file — enabled by default.
- Node.js 18 reached end-of-life in April 2025 and is the only active LTS line that does not have `require(esm)`.
...
If you are on Node.js **20 or 22**, nothing changes on the consumption side — `require('inversify')` will continue to work thanks to `require(esm)`.
