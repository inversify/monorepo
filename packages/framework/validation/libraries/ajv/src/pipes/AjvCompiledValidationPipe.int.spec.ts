...
import { ValidateAjvSchema } from '../decorators/ValidateAjvSchema.js';
import { AjvCompiledValidationPipe } from './AjvCompiledValidationPipe.js';
...
describe(AjvCompiledValidationPipe, () => {
  describe('having a AjvCompiledValidationPipe in an HTTP server with validated endpoints', () => {
    interface Message {
      content: string;
    }
    ...
    beforeAll(async () => {
      ...
      server = await buildExpressServer(
        ...
        [new AjvCompiledValidationPipe(new Ajv())],
      );
    });
