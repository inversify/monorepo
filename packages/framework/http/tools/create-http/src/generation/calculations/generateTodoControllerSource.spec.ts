...
describe(generateTodoControllerSource, () => {
  describe('when called', () => {
    ...
    it('should generate a TodoController with OpenAPI metadata and CRUD endpoints', () => {
      ...
      expect(result).toContain('@ValidatedBody() body: CreateTodoRequestBody');
      expect(result).toContain('@ValidatedBody() body: UpdateTodoRequestBody');
      ...
      expect(result).toContain('const pageSize: number = query.pageSize ?? 10');
      expect(result).toContain(
