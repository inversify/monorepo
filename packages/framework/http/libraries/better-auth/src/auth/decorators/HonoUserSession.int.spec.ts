...
import { BetterAuthHonoContainerModule } from '../services/BetterAuthHonoContainerModule';
...
describe(HonoUserSession, () => {
  ...
  beforeAll(async () => {
    const dbPath: string = './temp/hono-user-session.db';
  ...
  describe('having a Better Auth Hono server with session endpoint', () => {
    let server: Server;
