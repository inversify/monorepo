...
<Info>
  If you need full integration tests with real dependencies and actual
  InversifyJS containers, traditional container configuration may be more
  appropriate.
</Info>
...
<Info>
  Use `@suites/doubles.vitest` or `@suites/doubles.sinon` if you're using Vitest
  or Sinon instead of Jest.
</Info>
...
describe('Order Service Sociable Unit Spec', () => {
  ...
  it('should calculate and charge correct total', async () => {
    ...
    await orderService.processOrder([{ price: 10 }, { price: 20 }]);
