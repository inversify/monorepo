...
<Note>
  Private properties cannot be strongly typed due to TypeScript decorator
  limitations. Use public properties (optionally prefixed with underscore) or
  fall back to the regular `@inject()` decorator for private properties.
</Note>
...
const myModule = new TypedContainerModule<BindingMap>((bind) => {
