...
```ts
rebindAsync: <T>(serviceIdentifier: ServiceIdentifier<T>) =>
  Promise<BindToFluentSyntax<T>>;
```

Refer to the [docs](./container.mdx#rebindasync) for more information.
...
```ts
unbindAsync: (identifier: BindingIdentifier | ServiceIdentifier) =>
  Promise<void>;
```
